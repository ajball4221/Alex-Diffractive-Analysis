...
def t_Cut_Func(t_value):
    if t_value is not None:
        return t_value >= CutConfig.T_CUT
    else: return True
...
CUT_CONFIGS = {
    ...
    "Alex t Cut": {
        "value_getter": lambda event,nprong: TruthTools.tDef1_diff(event),
        # Gets t and returns None if not diffractive, and it passes the cut if it's not diffractive
	"cut_fn": lambda val: val >= CutConfig.T_CUT if val is not None else True,
    },
    "Alex Pion Energy Cut": {
        "value_getter": lambda event,nprong: TruthTools.PiZeroE_diff(event),
        # Gets t and returns None if not diffractive, and it passes the cut if it's not diffractive
        "cut_fn": lambda val: val >= CutConfig.PION_ENERGY_CUT if val is not None else True,
    },
    "Vertex_Z": {
    ...
    "PsiEe" : {
        "value_getter" : lambda event, nprong: event.ConeOutsideE()/1e3,
        "cut_fn" : lambda val: val < 0.45,
    },
    "InversePsiEe" : {
        "value_getter" : lambda event, nprong: event.ConeOutsideE()/1e3,
        "cut_fn" : lambda val: val > 0.45,
...
#       "cut_fn": lambda vals: vals[1] > 0 and vals[1] <= FRONTDEDX_CUT_GRAPH.Eval(vals[0]),
   ...
   "InverseMeanFrontdEdX": {
        "value_getter": lambda event, nprong: event.prong_dEdXMeanFrontTracker[nprong],
        "cut_fn": lambda val: val > CutConfig.FRONT_DEDX_CUT,
        "variable_range": [0.1* i for i in range(0,51)]
    }, 
    #   "UpstreamODEnergy": {
    ...
    "InlineUpstream" : {
        "value_getter": lambda event, nprong: event.UpstreamInlineEnergy,
        "cut_fn": lambda val: val > CutConfig.INLINE_CUT,
    },

    "InlineUpstream_Reverse" : {
        "value_getter": lambda event, nprong: event.UpstreamInlineEnergy,
        "cut_fn": lambda val: val < CutConfig.INLINE_CUT,
    },

    "InverseInlineUpstream" : {
        "value_getter": lambda event, nprong: event.UpstreamInlineEnergy,
        "cut_fn": lambda val: val < CutConfig.INLINE_CUT,
    },
    ...
    "WeightedInline" : {
        "value_getter" : lambda event, nprong: event.UpstreamInlineEnergyWgtdPosMean,
        "cut_fn" : lambda val: val > CutConfig.INLINE_WEIGHTED_CUT,
    },
    "InverseWeightedInline" : {
        "value_getter" : lambda event, nprong: event.UpstreamInlineEnergyWgtdPosMean,
        "cut_fn" : lambda val: val < CutConfig.INLINE_WEIGHTED_CUT,
    },
    "Neutrino Helicity" : {
