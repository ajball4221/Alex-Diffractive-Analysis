...
def ExponentialTruthFit(data,migration,efficiency,xname,yname): # These have to be the main data classes
    ...
    y_fitted_dist = fitter.y_bin_maker(result[0],result[1],reco = True)
...
def InverseMigration(data,migration,efficiency):
    ...
    for j in range(len(migration.values[0])):
       ...
       for i in range(len(migration.values)):
           error_sum += (migration.values[j,i]**2)*abs(data.values[i])
       reco_y_error[j] = np.sqrt(error_sum)
    #true_y_error = reco_y_error / efficiency.values
    return reco_y_dist, reco_y_error
...
def MatrixTruthRecovery(data,migration,efficiency,xname,yname): # These have to be the CV classes that include the universes
    ...
    for error_type in data.hist.GetErrorBandNames():
        ...
        for k in range(data.hist.GetVertErrorBand(error_type).GetNHists()):
            ...
            x_predicted_dist = migration.universes[error_type][k].values @ y_recovered_dist
            x_predicted_error = np.sqrt(np.diagonal(migration.universes[error_type][k].values @ np.power(np.diagflat(y_recovered_error),2) @ np.transpose(migration.universes[error_type][k].values)))
...
t_fitted_err.SetMarkerStyle(21)
...
ROOT.gPad.BuildLegend(0.45,0.35,0.85,0.55)
...
IUE_predicted_err.SetMarkerStyle(21)
...
IUE_matrixpredicted_err.SetMarkerStyle(21)
IUE_matrixpredicted_err.SetMarkerColor(4)
IUE_matrixpredicted_err.SetLineColor(9)

IUE_data_err = IUE.hist.GetCVHistoWithError()
IUE_data_err.SetMarkerStyle(21)
IUE_data_err.SetMarkerColor(8)
IUE_data_err.SetLineColor(8)
...
Eel_matrixpredicted_err.SetMarkerColor(4)
Eel_matrixpredicted_err.SetLineColor(9)
...
Eel_data_err.GetXaxis().SetTitle("Electron Cone Energy (GeV)")
