...
def MatrixTruthRecovery(data,migration,efficiency,xname,yname): # These have to be the CV classes that include the universes
    ...
    y_recovered_hist = PlotUtils.MnvH1D(str(yname)+"_recovered_sys","True "+str(yname)+" From Inverse Migration Matrix",len(migration.values),array("d",migration.ybins))
...
def NewMatrixTruthRecovery(data,migration,efficiency,xname,yname): # These have to be the CV classes that include the universes
    invmatrix = MPInverse(migration.values)
    reco_y_dist = invmatrix @ data.values
    true_y_dist = reco_y_dist / efficiency.values
    predicted_x_dist = migration.values @ reco_y_dist
    ...
    true_y_error = reco_y_error / efficiency.values
    predicted_x_error = np.sqrt(np.diagonal(new_cov_matrix_x))
    ...
    for j in range(len(predicted_x_dist)):
        x_predicted_hist.SetBinContent(j+1,predicted_x_dist[j])
        x_predicted_hist.SetBinError(j+1,predicted_x_error[j])
    chi2 = CalChi2(predicted_x_dist,data.error,data.values)
...
def HistCalChi2(data,prediction):
...
t_recovered_hist_sys = MatrixTruthRecovery(IUE,t_migration,t_efficiency,"IUE","t")
t_recovered_hist, IUE_matrixpredicted_hist = NewMatrixTruthRecovery(IUE,t_migration,t_efficiency,"IUE","t")
pion_fitted_hist, Eel_predicted_hist = ExponentialTruthFit(Eel,pion_migration,pion_efficiency,"Eel","pionE")
pion_recovered_hist_sys = MatrixTruthRecovery(Eel,pion_migration,pion_efficiency,"Eel","pionE")
pion_recovered_hist, Eel_matrixpredicted_hist = NewMatrixTruthRecovery(Eel,pion_migration,pion_efficiency,"Eel","pionE")
...
t_recovered_hist_sys.Write()
pion_recovered_hist_sys.Write()
