...
def MPInverse(matrices):
    svds = []
    discard_ratio = 100  # Ratio at which an singular value will be removed
    ...
    discarded = np.zeros((len(svds),len(svds[0][1])),dtype = bool)
    for j,svd in enumerate(svds):
        largestSV = svd[1][0] # If the ratio between the largest and current SV is too large, the SV gets set to 0
        for i in range(1,len(svd[1])):
    ...
    print(discarded)
    np.savetxt("/minerva/app/users/ajball/ncdiff/CC-NuE-XSec/tools/discarded.txt",discarded,delimiter=",",fmt = "%d")
    return inverses
...
weights = DoLinearAlgebra(MPInverse(matrices),vectors)
